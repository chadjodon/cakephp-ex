...
if ($sepDirs != NULL && count($sepDirs)>0) {
   $linkToDirs = "<a href=\"".$GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']."admincontroller.php?action=".getParameter("action")."&orderby=".$orderby."&theme=".$theme."&advsearch=".$advsearch."&searchstr=".$searchstr."\">root/</a>";
   $tempDir = "";
   for ($i=0; $i<(count($sepDirs)-1); $i++) {
      $tempDir .= $sepDirs[$i]."/";
      $linkToDirs .= "<a href=\"".$GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']."admincontroller.php?action=".getParameter("action")."&orderby=".$orderby."&curdir=".$tempDir."&theme=".$theme."&advsearch=".$advsearch."&searchstr=".$searchstr."\">".$sepDirs[$i]."/</a>";
   }
...
if ($advsearch==1) {
   ...
   if ($theme > 0) {
      ...
      $link = "<a href=\"".$GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']."admincontroller.php?action=".getParameter("action")."&orderby=".$orderby."&curdir=".$curdir."&cmsid=".$cmsid."&version=".$version."&advsearch=".$advsearch."&searchstr=".$searchstr."\">";
...
if ($searchstr != NULL) {
   $link = "<a href=\"".$GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']."admincontroller.php?action=".getParameter("action")."&orderby=".$orderby."&curdir=".$curdir."&cmsid=".$cmsid."&version=".$version."&advsearch=".$advsearch."&theme=".$theme."\">";
   $link .= "<img src=\"".$GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']."images/close_btn.gif\" border=\"0\"> ";
   $link .= "</a> ";
...
<?php
      ...
      $disabled="";
      if (0==strcmp($cmsfver['status'],"ACTIVE") || (!$ua->doesUserHaveAccessToLevel(isLoggedOn(),3) && !$ua->isUserAccessible(isLoggedOn(),"CMS",$cmsid))) $disabled="DISABLED";
?>
      <table width="100%" cellpadding="1" cellspacing="1" border="0" bgcolor="white">
      <form action="<?php echo $GLOBALS['baseURLSSL'].$GLOBALS['adminFolder']; ?>admincontroller.php" name="versioncontrol" method="POST">
      <input type="hidden" name="action" value="showversionfiles">
...
?>      
               <tr><td><b>Shortname</b></td><td><b>Filename</b></td><td><b>title</b></td><td></td></tr>
<?php 
                for ($i=0; $i<count($files); $i++) {
                  $file = $files[$i];
